...
- **行动引导的无效工具参数现在会自动恢复。** `requireToolUse` runtime 遇到语法损坏的 tool arguments 时，会丢弃无法被 OpenAI-compatible provider 接受的坏 transcript，注入一次定向 JSON 纠正并在正常 step 预算外自动重试；`guide` 同时恢复 transient LLM 重试，并把工具参数收窄为固定的 `topic`、`safe`、`aggressive`、`creative` 扁平结构，避免玩家再靠手动重跑修复 `arguments are not valid JSON`。
- **默认文本模型现在使用 xAI Grok 4.6，行动引导获得更长恢复窗口。** 缺少 `llm.toml` 时，内置 `story` 用途改为 `xai/grok-4.6`，首次配置也默认选择 xAI；FrostFox 托管目录在管理员未指定调度计划时优先选择可用的 Grok 4.6，并同步绑定 `story`、`plugin`、`utility` 与 `default`。`guide` 的 runtime / 单次调用上限分别从 90 / 45 秒提高到 180 / 90 秒，保留一次自动重试。
...
- **托管模型管理员权限保持一致。** 已授权运维账号与 FrostFox 管理员都能编辑模型用途、运行时模型覆盖和共享故障转移计划；账号页明确显示“模型管理员”，避免服务端已授权而前端仍错误锁定。
- **插画生成现在有实时弹窗反馈。** 场景背景与图像插件通过 kernel job-status 上报生成状态；当前回合开始绘图时自动显示进度弹窗，关闭后任务继续在后台运行，完成或失败状态都会明确反馈。
- **Agent runtime 流式调用按响应字节续期。** 无结构化输出的插件现在也使用 provider 流式接口；持续生成 reasoning 或工具参数时会续期调用空闲窗口，但仍受 runtime 总时长硬上限约束，插件内部文本不会进入玩家叙事流。
- **Runtime failures now survive reconnects.** Returned agent failures and pre-runtime vetoes publish a terminal callback, action traces persist canonical `runtime.failed` rows, and snapshot/local-cache restore preserves `failed` / `skipped` / `suspended` statuses instead of turning generic `runtime.completed` records into green completions.
