...
- `COVEL_SYSTEM_PROXY_URL` 由 Electron 主进程调用 Chromium 系统代理解析后注入 sidecar。设置选择 `system` 时使用该 URL；空值表示系统规则返回 `DIRECT`。该值只用于核心 provider 与模型数据库请求，不应用到第三方插件的 `fetchWithRetry`。
