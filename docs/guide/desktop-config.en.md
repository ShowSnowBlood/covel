...
See [`llm.toml.example`](../../llm.toml.example) at repo root. Each model role pairs a provider and model. The built-in fallback `story` role uses xAI `grok-4.6`; filling `XAI_API_KEY` in `keys.env` is enough to run it.
