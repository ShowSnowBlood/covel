...
- [modules/package-runtime/src/manifest.ts](REPO_ROOT/modules/package-runtime/src/manifest.ts)
  ...
  - 新增 top-level `state`
- [modules/package-runtime/src/runtime.ts](REPO_ROOT/modules/package-runtime/src/runtime.ts)
  - 新增 `listHooks()`
  - 新增 `listCapabilities()`
  - 新增 `getBlockType()`
...
- [modules/domain/src/entities.ts](REPO_ROOT/modules/domain/src/entities.ts)
  - `World` 增加 `taskBindings?: Record<string, string>`
  - `Session` 增加 `taskBindings?: Record<string, string>`
  - 新增 `PackageStateRecord`
  - 新增 `JobRecord`
- [modules/domain/src/repositories.ts](REPO_ROOT/modules/domain/src/repositories.ts)
...
- [modules/storage/src/in-memory-storage-repositories.ts](REPO_ROOT/modules/storage/src/in-memory-storage-repositories.ts)
  - 实现 package state
  - 实现 job repository
  - pending block 改为保存完整 envelope
- [modules/storage/src/postgres-storage-port.ts](REPO_ROOT/modules/storage/src/postgres-storage-port.ts)
  - 新增 package state 表
  - 新增 jobs 表
  - pending block 表增加：
    - block envelope json
...
- [modules/contracts/src/block.ts](REPO_ROOT/modules/contracts/src/block.ts)
  - `meta` 增加 `handler`
  - `interaction` 增加 package-owned resume metadata
  - 允许 `data` 和 `response` 走更严格 typed contract
- [modules/contracts/src/action-request.ts](REPO_ROOT/modules/contracts/src/action-request.ts)
...
- [modules/model-gateway/src/model-profile-registry.ts](REPO_ROOT/modules/model-gateway/src/model-profile-registry.ts)
  - `SupportedMode` 扩成：
    - `text`
    - `object`
    - `stream`
    ...
    - `speech`
    - `transcription`
- [modules/model-gateway/src/provider-registry.ts](REPO_ROOT/modules/model-gateway/src/provider-registry.ts)
  - adapter 接口增加：
    - `generateImage`
    - `synthesizeSpeech`
    - `transcribeAudio`
- [modules/model-gateway/src/runtime.ts](REPO_ROOT/modules/model-gateway/src/runtime.ts)
  - gateway 增加对应方法
...
当前建议直接重写 [modules/flow-engine/src/runtime.ts](REPO_ROOT/modules/flow-engine/src/runtime.ts)。
...
- [apps/runtime/src/composition.ts](REPO_ROOT/apps/runtime/src/composition.ts)
  - 组合：
    - package host
    - task router
    - capability runtime
...
- [apps/runtime/src/server.ts](REPO_ROOT/apps/runtime/src/server.ts)
  - `/actions` 继续保留
...
- [apps/web/src/App.tsx](REPO_ROOT/apps/web/src/App.tsx)
  - 去掉硬编码 `choices` block 渲染
  - 改为 renderer registry
- [apps/web/src/types.ts](REPO_ROOT/apps/web/src/types.ts)
  - 补 artifact / job / richer block types
- [apps/web/src/state.ts](REPO_ROOT/apps/web/src/state.ts)
...
- [extensions/core-guide/client/renderers/choices.tsx](REPO_ROOT/extensions/core-guide/client/renderers/choices.tsx)
  - 当前 placeholder 不能再被当成“renderer 能力已存在”
- [extensions/*/server/context.ts](REPO_ROOT/extensions/core-guide/server/context.ts)
  - 当前 `return []` 的 context 文件不能再继续扩散
- [modules/flow-engine/src/runtime.ts](REPO_ROOT/modules/flow-engine/src/runtime.ts)
  - 当前恢复逻辑不能再沿用
