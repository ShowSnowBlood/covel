...
| `runtime.completed`   | S→C  | 单个 runtime 完成 | `{ runtimeId, pluginId, status, durationMs }`；`status` 可为 `success`、`skipped` 或 `suspended`，兼容失败结果也可能为 `failed`                                                                                                                                                                                       |
| `runtime.failed`      | S→C  | 单个 runtime 失败 | `{ runtimeId, pluginId, status, error, durationMs? }`                                                                                                                                                                                                                                                                 |
...
`DEPLOYMENT_TIER=demo|commercial` 时该端点执行 hosted 会话鉴权：owner token、运维 token，或启用 FrostFox SaaS 时匹配会话归属的账号 Cookie 三者之一即可。内置 Web 使用 fetch-based SSE 并提交 `X-Session-Token`；原生 `EventSource` 客户端可用 `?session_token=<ownerToken>`。缺失或错误返回 `401 { code: "session_owner_required" }`。`self`（默认）层级不强制。详见 [`docs/reference/api.md`](./api.md) 鉴权章节。
