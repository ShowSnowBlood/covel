...
**旧 inline 数据处理方式**（2026-04-26 用户决策）：历史 base64 / data URI 数据采用一次性重置策略，新版本生成结果全部通过 `MediaRef` 写入 MediaStore。开发机直接删除 `~/.covel/data/covel.db*` 和旧插件 `.bak-p0c-*` 目录，启动后由当前 schema 重建 DB。
