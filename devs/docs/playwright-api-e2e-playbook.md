...
- For live AI world generation, confirm the active provider config through `/api/config/info`; the dev server can use `~/.covel/llm.toml` instead of the repo-local `llm.toml`. A failed `/api/ai/ping` without browser provider headers is not enough to prove generation auth failure; check `logsDir/server.log` for `ai/generate-world`, `LLM responded`, parse, validation, and timeout entries.
